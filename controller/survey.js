...
exports.edit = function(req, res){
    ...
    if (req.method == 'POST') {
        ...
        var cause_id = post.causeId;
        var title = post.title;
        console.log("titleee", req.body,title);
        title = title.replace(/'/g, '\\\'');
...
exports.delete = function(req,res) {
    ...
    var sql = "delete FROM `survey` where `id` =" + id;
    ...
    mysql.createConnection(config.dbConfig).then(function(conn){
        ...
        sql = "delete FROM `surveyquestions` where SurveyId =" + id;
        result = connection.query(sql);

        return result;
    }).then(function(rows) {
        res.cookie('message', 'Survey is deleted successfully')
        res.cookie('icon', 'success')
        res.cookie('heading', 'Success')
        res.send({status: 'success'});
    })
